...
// Defines base structure per category per tier.
// Trait scoring reorders within this base.
// -----------------------------
const BLUEPRINT_MAP = {
  hair_beauty: {
...
function ensureCore(layout) {
  const withHero = layout.filter(m => m === "hero");
  const withCta = layout.filter(m => m === "cta");
  const middle = layout.filter(m => m !== "hero" && m !== "cta");

  // hero always first, cta always last
  return [
    ...(withHero.length ? ["hero"] : ["hero"]),
    ...middle,
    ...(withCta.length ? ["cta"] : ["cta"])
  ];
}
...
// -----------------------------
// TRAIT SCORING
// Uses unified vocabulary from taxonomy.js.
// Higher score = module appears earlier/more likely to survive budget cut.
// -----------------------------
...
function applyModifiers(layout, modifiers = [], tier) {
  ...
  if (mods.includes("booking_driven") && !layout.includes("booking")) {
    layout.push("booking");
  }
...
export function generateBlueprint({
  ...
  // -----------------------------
  // 1. BASE STRUCTURE FROM CATEGORY MAP
  // -----------------------------
  let layout =
  ...
  // Modules with high trait score bubble up.
  // hero and cta are re-pinned after sort.
  // -----------------------------
  const core = layout.filter(m => CORE_REQUIRED.includes(m));
